...
func NewDailyLogger(ctx context.Context, dir string, namef string, h int, m int, outs ...io.Writer) (*log.Logger, error) {
	err := prepareDir(dir)
	if err != nil {
		return nil, err
	}
	...
	// and repeat
	go repeat(ctx, d, dir, namef, l, outs...)

	return l, nil
}

func repeat(ctx context.Context, d time.Duration, dir string, namef string, l *log.Logger, outs ...io.Writer) {
	var prevf, f *os.File
	prevf = f
	for {
		select {
...
func openFile(dir string, namef string) (*os.File, error) {
	t := time.Now()

	// check if a file with today's date exists
	// if yes, return it
	fname := filepath.Join(dir, t.Format(namef))
